/*
	Bind() 에러 발생.
	TCP의 종료시점에서 4-hand shaking 작업 후
	time-wait 시간이 있는데, 이때문에 소켓을 잠시동안 쓸 수 없음.
	먼저 종료를 요청한 호스트가 time-wait를 갖음.
	...
	A host                     B host
	  ...
	  |<------ACK----------------| 받았다 응답함
	  |                          |
	  |<------FIN,ACK------------| B도 finish 신호를 보내고
	  |                          |
	  |---------ACK------------->| finish 신호에 대한 응답을 받았으니 종료함.
	  |                           
	  |                          A는 혹시 ACK가 네트워크 끊김으로 보내지지 않았을 경우를 대비해 timewait 상태로 좀 기다렸다가 끝냄. 
	  |                          
